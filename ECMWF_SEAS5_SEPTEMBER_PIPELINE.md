...
| **Step 8.4** | Dashboard did not update when selecting Short Rains (Vuli: Oct-Dec) 2025 | 1. Backend grid/pixel endpoints did not accept `season` param and defaulted to MAM.<br>2. `usePixelStats` and `useGrid` query hooks did not include `season` in cache keys.<br>3. Plume charts were hardcoded to MAM window (DOY 32–181) and sliced array out of bounds for SOND (DOY 244–365).<br>4. Climatological anomaly was subtracting MAM baseline from SOND values (+237d anomaly). | 1. Connected `outputs/ecmwf_sep` to `backend/mam_loader.py` with dynamic seasonal CHIRPS climatology.<br>2. Added `season` param to FastAPI `/grid` and `/pixel` endpoints.<br>3. Updated React query hooks to include `season` and invalidate cache appropriately.<br>4. Configured dynamic DOY axes and month labels (`DOY_MONTHS_SOND`: Sep, Oct, Nov, Dec vs `DOY_MONTHS_MAM`) and fixed member array slicing.<br>5. Tested with automated TestClient suite (`test_api_seasons.py`) and verified production build (`npm run build`). | ✅ Resolved |
